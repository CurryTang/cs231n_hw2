...
def affine_forward(x, w, b):
    ...
    ###########################################################################
    x_new = x.reshape([x.shape[0], np.product(x.shape[1:])])
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        nor_x = (x - batch_mean) / np.sqrt(eps + batch_var)
        trans_x = nor_x * gamma + beta
        cache = (gamma, x, batch_mean, batch_var, eps, nor_x)
        out = trans_x
...
def batchnorm_backward(dout, cache):
    ...
    # might prove to be helpful.                                              #
    ###########################################################################
    gamma, x, mean, var, eps, trans_x = cache
    dtrans_x = dout * gamma
    dsample_var = np.sum(dtrans_x * (-0.5 * trans_x / (var + eps)), axis = 0)
    dmean = np.sum(dtrans_x * -1 / np.sqrt(var + eps), axis = 0) + dsample_var * np.sum(-2 / N * (x - mean), axis = 0)
    dx_part1 = dtrans_x * 1 / np.sqrt(var + eps)
    dx_part2 = dsample_var * 2 * (x - mean) / N
    dx_part3 = (dmean / N)
    dbeta = np.sum(dout, axis = 0)
    dgamma = np.sum(dout * trans_x, axis = 0)
...
def batchnorm_backward_alt(dout, cache):
    ...
    dx = (1. / N) * gamma / np.sqrt(var + eps) * (N * dout - np.sum(dout, axis=0)
                                                       - (x - mean) / (var + eps) * np.sum(dout * (x - mean), axis=0))
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def layernorm_forward(x, gamma, beta, ln_param):
    ...
    ###########################################################################
    ## BatchNorm是一个batch里在N个维度上做平均，ＬａｙｅｒＮｏｒｍ是在一个data内部的几个dimension上做平均
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    # Zero padding for the higher dimension, and pad padding for length and width
    padded_X = np.pad(x, ((0, 0), (0, 0), (pad, pad), (pad, pad)), "constant")
    out = np.random.rand(N, F, H_prime, W_prime)
    for i in range(H_prime):
        for j in range(W_prime):
                for m in range(N):
                    for k in range(F):
                        out[m, k, i, j] = np.sum(padded_X[m, :, i * stride:i * stride+HH, j * stride:j*stride+WW] * w[k, :, :, :]) + b[k]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, w, b, conv_param)
        ...
        for b in range(F):
            for j in range(H_out):
                for k in range(W_out):
                    dx_pad[a, :, j * stride:j * stride+HH, k * stride:k * stride + WW] +=  w[b] * dout[a, b, j, k]
                    ## 跟三层都有关系，所以x的第二层是:
                    # print("The shape of dw[b] is {}, \n The shape of x is {} \n The shape of dout is {}".format(
                    #     dw[b].shape, x.shape, dout[a,b,j,k].shape
                    # ))
                    dw[b] += x_pad[a, :, j * stride:j * stride + HH, k * stride:k * stride + WW] * dout[a, b, j, k]
...
def max_pool_forward_naive(x, pool_param):
    ...
    W_out = 1 + int((H - pool_width) / stride)
        ...
        for j in range(C):
            for m in range(H_out):
                for n in range(W_out):
                    window = x[i, j, m * stride:m*stride+pool_height, n * stride:n*stride+pool_width]
                    dx[i, j, m * stride:m*stride+pool_height,n * stride:n * stride+pool_width] = (window == np.max(window)) * dout[i, j, m, n]
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    trans_x = trans_x.reshape(np.product(trans_x.shape[:3]), trans_x.shape[-1])
...
def spatial_batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    N, C, H, W = dout.shape
    trans_dout = dout.transpose(0,2,3,1).reshape(-1, C)
    dx, dgamma, dbeta = batchnorm_backward_alt(trans_dout, cache)
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    trans_x = x_centered * i_std

    trans_x = trans_x.reshape(N, C, H, W)

    out = gamma * trans_x + beta

    cache = (trans_x, gamma, i_std, G)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return out, cache
