...
from cs231n.layer_utils import affine_bn_relu_forward
from cs231n.layer_utils import affine_bn_relu_backward
...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        scores, final_cache = affine_relu_forward(out, self.params['W2'], self.params['b2'])
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        ############################################################################
        softmaxloss, dscores = softmax_loss(scores, y)
        loss = softmaxloss + 0.5 * self.reg * (np.sum(self.params['W1'] * self.params['W1']) + np.sum(self.params['W2'] * self.params['W2']))
        dy, grads['W2'], grads['b2'] = affine_relu_backward(dscores, final_cache)
        dinput, grads['W1'], grads['b1'] = affine_relu_backward(dy, cache)
...
#         self.dtype = dtype
#         self.params = {}
#         self.dims = np.insert(hidden_dims, 0, input_dim)
#         self.dims = np.append(self.dims, num_classes)
#         ############################################################################
...
#             self.params['W{}'.format(i + 2)] = np.random.normal(loc = 0, scale = weight_scale, size = (hidden_dims[i], hidden_dims[i + 1]))
#             self.params['b{}'.format(i + 2)] = np.zeros(shape = hidden_dims[i + 1])
...
#             self.params['W{}'.format(len(hidden_dims) + 1)] = np.random.normal(loc = 0, scale = weight_scale, size = (hidden_dims[-1], num_classes))
#             self.params['b{}'.format(len(hidden_dims) + 1)] = np.zeros(shape=num_classes)
...
#             self.bn_params = [{'mode': 'train', 'running_mean':np.zeros(self.dims[i + 1]), 'running_var':np.zeros(self.dims[i + 1])} for i in range(self.num_layers - 1)]
...
#             # Tracer()()
#             current_W = self.params['W{}'.format(i + 1)]
#             current_b = self.params['b{}'.format(i + 1)]
#             current_gamma = self.params['gamma{}'.format(i + 1)]
#             current_beta = self.params['beta{}'.format(i + 1)]
#             if (self.normalization == "batchnorm"):
#                 temp, cache = affine_bn_relu_forward(temp, current_W, current_b, current_gamma, current_beta, self.bn_params[i])
#             else:
#                 temp, cache = affine_relu_forward(temp, current_W, current_b)
#             reg_num += 0.5 * self.reg * np.sum(current_W * current_W)
#             gradient_cache['cache{}'.format(i + 1)] = cache
#         final_W = self.params['W{}'.format(self.num_layers)]
#         final_b = self.params['b{}'.format(self.num_layers)]
#         temp, cache = affine_forward(temp, final_W, final_b)
#         gradient_cache['cache{}'.format(self.num_layers)] = cache
#         reg_num += 0.5 * self.reg * np.sum(final_W * final_W)
#         scores = temp
...
#             current_cache = gradient_cache['cache{}'.format(i)]
#             if (i == self.num_layers):
#                 dout, grads['W{}'.format(i)], grads['b{}'.format(i)] = affine_backward(dout, current_cache)
#             else:
#                 if (self.normalization == "batchnorm"):
#                     dout, dw, db, dgamma, dbeta = affine_bn_relu_backward(dout, current_cache)
#                     grads['W{}'.format(i)] = dw
#                     grads['b{}'.format(i)] = db
#                     grads['gamma{}'.format(i)] = dgamma
#                     grads['beta{}'.format(i)] = dbeta
#                 else:    
#                     dout, grads['W{}'.format(i)], grads['b{}'.format(i)] = affine_relu_backward(
#                         dout, current_cache
#                     )
#         for i in range(1, self.num_layers + 1):
#             current_W = self.params['W{}'.format(i)]
#             grads['W{}'.format(i)] += self.reg * current_W
#         ############################################################################
#         #                             END OF YOUR CODE                             #
#         ############################################################################
